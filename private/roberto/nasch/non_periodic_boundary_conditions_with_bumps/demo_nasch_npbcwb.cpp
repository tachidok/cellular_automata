            ...
            for (double alpha = alpha_min; alpha <= alpha_max; alpha+=alpha_step)
              {
                ...
                //std::vector<unsigned> bumps_positions{LANE_SIZE/2 - 1, LANE_SIZE/2, LANE_SIZE/2 + 1};
                std::vector<unsigned> bumps_positions{LANE_SIZE/2};
                ...
                for (unsigned i = 0; i < monte_carlo_max_loop; i++)
                  {
                    ...
                    //crosswalk.update_pedestrian_list();


                    //crosswalk.apply_motion_rules();
